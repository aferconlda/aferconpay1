...
class _PayWithQrScreenState extends State<PayWithQrScreen> {
  ...
  Future<void> _loadInitialData() async {
    try {
      ...
      if (authUser == null) throw Exception('Utilizador não autenticado.');
      ...
      if (recipient == null) throw Exception('O destinatário do pagamento não foi encontrado.');
      if (currentUser == null) throw Exception('Não foi possível carregar os seus dados.');
      if (recipient.uid == currentUser.uid) throw Exception('Não pode fazer um pagamento a si mesmo.');
  ...
  Future<void> _processPayment() async {
    if (!(_formKey.currentState?.validate() ?? false) || _recipient == null) return;
                        ...
                        child: Column(
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(color: theme.cardColor, borderRadius: BorderRadius.circular(20), border: Border.all(color: theme.primaryColor.withAlpha(77))),
                              child: Column(
                                children: [
                                  Text('VOCÊ ESTÁ PAGANDO PARA', style: theme.textTheme.labelLarge?.copyWith(color: theme.colorScheme.secondary, fontWeight: FontWeight.bold, letterSpacing: 0.8)),
                                  const SizedBox(height: 16),
                                  CircleAvatar(radius: 35, backgroundColor: theme.colorScheme.primary, child: Text(recipientName.isNotEmpty ? recipientName.substring(0, 1).toUpperCase() : 'U', style: theme.textTheme.headlineMedium?.copyWith(color: Colors.white, fontWeight: FontWeight.bold))),
                                  const SizedBox(height: 12),
                                  Text(recipientName, style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                                ],
                              ),
                            ),
                            Padding(
                              ...
                              child: Column(
                                children: [
                                  TextFormField(
                                    ...
                                    decoration: InputDecoration(labelText: 'Valor a Pagar', labelStyle: TextStyle(color: theme.colorScheme.primary), suffixText: 'Kz', border: const OutlineInputBorder(), filled: false),
                                    ...
                                    validator: (value) {
                                      ...
                                      if (amount > balance) return 'Saldo insuficiente. Saldo atual: $balance Kz';
                                      return null;
                                    },
                                  ),
                                  const SizedBox(height: 24),
                                  TextFormField(controller: _descriptionController, textAlign: TextAlign.center, decoration: InputDecoration(labelText: 'Descrição', labelStyle: TextStyle(color: theme.colorScheme.primary), border: const OutlineInputBorder(), filled: false)),
                                ],
                              ),
                            ),
